...
const int    nbins     = 10;
const int    nbins2    = 5;
const double bins[11]  = {-2.4 , -2.0 , -1.5 , -1.0 , -0.5 , 0.0 , 0.5 , 1.0 , 1.5 , 2.0 , 2.4};
const double bins2[6] = {0.0 , 0.5 , 1.0 , 1.5 , 2.0 , 2.4};
  ...
  if (doReBin) {
    ...
    for (uint i = 1; i <= nbins; i++) {
      ...
      for (const auto& iH : iHV) {
        if (iHV.size()>1 && bins[i-1]>=PA::EtaCMtoLAB(double(iH-6), true)) continue;
        if (iHV.size()>1 && bins[i]  <=PA::EtaCMtoLAB(double(iH-7), true)) break;
        ...
        for (int j = 1; j <= hSet->GetNbinsX(); j++) {
          if ( (PA::EtaCMtoLAB(hSet->GetBinCenter(j), true) > bins[i-1]) && (PA::EtaCMtoLAB(hSet->GetBinCenter(j), true) < bins[i]) ) {
...
void getCrossSection(TH1F& h, const TH1F& hYl, const bool isEPS09)
{
...
void getChargeAsymmetry(TH1F& h, const TH1F& hPl, const TH1F& hMi, const bool isEPS09)
{
...
void getForwardBackwardRatio(TH1F& h, TH1F& hYl, const bool isEPS09)
{
...
void getForwardBackwardRatio(TH1F& h, TH1F& hPl, TH1F& hMi, const bool isEPS09)
{
...
bool getPDFHessianError(TGraphAsymmErrors& gr, const std::vector< TH1F >& hVec, const double CL=90.)
{
  ...
  for (uint i = 0; i < nBin; i++) {
    ...
    getHessianError(yErrLo, yErrHi, yErrSy, y, CL);
    // Set the point
...
bool getPDFHessianCorrelation(double& corr, const int binA, const int binB, const std::vector< TH1F >& hVecA, const std::vector< TH1F >& hVecB)
{
  ...
  for (uint j=0; j < hVecA.size(); j++) { valuesA.push_back(hVecA[j].GetBinContent(binA)); }
  std::vector< double > valuesB;
  for (uint j=0; j < hVecB.size(); j++) { valuesB.push_back(hVecB[j].GetBinContent(binB)); }
  // Get correlation
  corr = getHessianCorrelation(valuesA, valuesB);
  // Return
  return true;
};
