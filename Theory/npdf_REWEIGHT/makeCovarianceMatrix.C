...
void makeCovarianceMatrix()
{
  ...
  for (uint i = 0; i < NCT14; i++) {
    PDFSetName["CT14"]["Pl"].push_back(Form("MCFM/CT14nlo_rewt_Wxsec/W_only_nlo_CT14nlo_80___80___W1_nlo_CT14nlo_rewt_%d.root", i));
    PDFSetName["CT14"]["Mi"].push_back(Form("MCFM/CT14nlo_rewt_Wxsec/W_only_nlo_CT14nlo_80___80___W6_nlo_CT14nlo_rewt_%d.root", i));
  }
  PDFSetName["nCTEQ15"]["Pl"].push_back(Form("MCFM/nCTEQ15nlo_rewt_Wxsec/W_only_nlo_CT14nlo_80___80___W1_nlo_nCTEQ15nlo_rewt_%d_0.root",0));
  PDFSetName["nCTEQ15"]["Mi"].push_back(Form("MCFM/nCTEQ15nlo_rewt_Wxsec/W_only_nlo_CT14nlo_80___80___W6_nlo_nCTEQ15nlo_rewt_%d_0.root",0));
  for (uint i = 1; i < NnCTEQ15; i++) {
    PDFSetName["nCTEQ15"]["Pl"].push_back(Form("MCFM/nCTEQ15nlo_rewt_Wxsec/W_only_nlo_CT14nlo_80___80___W1_nlo_nCTEQ15nlo_rewt_0_%d.root",i));
    PDFSetName["nCTEQ15"]["Mi"].push_back(Form("MCFM/nCTEQ15nlo_rewt_Wxsec/W_only_nlo_CT14nlo_80___80___W6_nlo_nCTEQ15nlo_rewt_0_%d.root",i));
  }
  if (addCT14VAR) {
    for (uint i = 1; i < NCT14; i++) {
      PDFSetName["nCTEQ15"]["Pl"].push_back(Form("MCFM/nCTEQ15nlo_rewt_Wxsec/W_only_nlo_CT14nlo_80___80___W1_nlo_nCTEQ15nlo_rewt_%d_0.root",i));
      PDFSetName["nCTEQ15"]["Mi"].push_back(Form("MCFM/nCTEQ15nlo_rewt_Wxsec/W_only_nlo_CT14nlo_80___80___W6_nlo_nCTEQ15nlo_rewt_%d_0.root",i));
    }
  }
  PDFSetName["EPPS16"]["Pl"].push_back(Form("MCFM/EPPS16nlo_rewt_Wxsec/W_only_nlo_CT14nlo_80___80___W1_nlo_EPPS16nlo_rewt_%d_%d.root",0,0));
  PDFSetName["EPPS16"]["Mi"].push_back(Form("MCFM/EPPS16nlo_rewt_Wxsec/W_only_nlo_CT14nlo_80___80___W6_nlo_EPPS16nlo_rewt_%d_%d.root",0,0));
  for (uint i = 1; i < (NEPPS16-NCT14+1); i++) {
    const int i1 = (i<=40) ? 0 : i-40;
    PDFSetName["EPPS16"]["Pl"].push_back(Form("MCFM/EPPS16nlo_rewt_Wxsec/W_only_nlo_CT14nlo_80___80___W1_nlo_EPPS16nlo_rewt_%d_%d.root",i1,i));
    PDFSetName["EPPS16"]["Mi"].push_back(Form("MCFM/EPPS16nlo_rewt_Wxsec/W_only_nlo_CT14nlo_80___80___W6_nlo_EPPS16nlo_rewt_%d_%d.root",i1,i));
  }
  if (addCT14VAR) {
          ...
          for (int i2 = 0; i2 < nChg; i2++) {
            ...
            for (int j1 = 0; j1 < nBin; j1++) {
              for (int j2 = 0; j2 < nBin; j2++) {
                ...
                double corr12  = 1.0;
                if (j1!=j2 || i1!=i2) { if (!getPDFHessianCorrelation(corr12, (j1+1), (j2+1), pdf1, pdf2)) { return; } }
...
void drawChi2Hist(const Chi2VecMap& Chi2ObsVec, const std::string& outDir)
{
  ...
  for (const auto& var : Chi2ObsVec.at("EPPS16")) {
    for (const auto& chg : var.second) {
      ...
      if (chg.first == "Pl") { sampleLabel = "W^{#pm} #rightarrow #mu^{#pm} + #nu_{#mu}"; }
      ...
      for (auto& gr : graph) {
        // General
        gr.second.SetTitle(";#chi^{2}/ndf;PDF sets");
      ...
      //
      // Draw graph
      graph.at("EPPS16").Draw();
      graph.at("nCTEQ15").Draw("same");
      graph.at("CT14").Draw("same");
      //
