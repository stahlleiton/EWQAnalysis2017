...
void makeCovarianceMatrix()
{
  ...
  //
  std::cout << "[INFO] Starting the covariance test" << std::endl;
  if (true) {
  for (const auto& mod : CovMatrixObs) {
    if (mod.first!="DATA" && mod.first!="nCTEQ15" && mod.first!="EPPS16" && mod.first!="CT14") continue;
    if (mod.first!="nCTEQ15" && mod.first!="CT14") continue;
    ...
    // Generate MC samples
    std::vector<TVectorD> genPars;
    TDecompChol choMatrix(covMatrixX);
    bool genPass = choMatrix.Decompose();
    // Correct covariance if not positive-definite
    if (!genPass) {
      int k = 0;
      TMatrixDSym covMatrixXCorr(covMatrixX);
      while (genPass==false && k++<50) {
        const TMatrixDSymEigen eigenvariances(covMatrixXCorr);
        const auto& eigenVal = eigenvariances.GetEigenValues();
	double minEigVal = 99999.; for (uint iT=0; iT<nColX; iT++) { if (minEigVal>eigenVal[iT]) { minEigVal = eigenVal[iT]; } }
	std::cout << "[INFO] Eigenvalue: " << minEigVal << std::endl;
        TMatrixDSym d; d.ResizeTo(covMatrixX); for (uint iX=0; iX<nColX; iX++) { d[iX][iX] = 2.0*std::abs(minEigVal) + 1.E-17; }
        covMatrixXCorr += d;
        choMatrix.SetMatrix(covMatrixXCorr);
        genPass = choMatrix.Decompose();
      }
    }
    if (!genPass) { std::cout << "[ERROR] Cholesky decomposition failed for " << mod.first << " !" << std::endl; continue; }
    const TMatrixD Ut(TMatrixD::kTransposed,choMatrix.GetU());
    for (uint iSmp=0; iSmp<2000000; iSmp++) {
      TVectorD tmp(nColX);
      for (uint iX=0; iX<nColX; iX++) { tmp[iX] = gRandom->Gaus(); }
      tmp *= Ut;
      tmp += nomVecX;
      genPars.push_back(tmp);
    }
    //
    for (const auto& var : mod.second) {
      for (const auto& chg : var.second) {
	...
	// Compute observables
	std::vector<TVectorD> genObs;
        for (uint iSmp=0; iSmp<genPars.size(); iSmp++) { TVectorD tmp; tmp.ResizeTo(nomVecT); tmp.Zero(); genObs.push_back(tmp); }
	if (var.first=="Cross_Section") continue;
	else {
	  for (uint iT=0; iT<nColT; iT++) {
	    if (var.first=="ForwardBackward_Ratio" && chg.first=="Pl") {
	      const uint iXFW = ((iT<nColT/2) ? 14 : 28) + iT;
              const uint iXBW = ((iT<nColT/2) ? 13 : 47) - iT;
	      nomVecT[iT] = nomVecX[iXFW]/nomVecX[iXBW];
	      for (uint iSmp=0; iSmp<genPars.size(); iSmp++) {
	        genObs[iSmp][iT] = genPars[iSmp][iXFW]/genPars[iSmp][iXBW];
	      }
	    }
	    else if (var.first=="ForwardBackward_Ratio" && chg.first=="Inc") {
	      ...
	      for (uint iSmp=0; iSmp<genPars.size(); iSmp++) {
		genObs[iSmp][iT] = (genPars[iSmp][iXFWPl]+genPars[iSmp][iXFWMi])/(genPars[iSmp][iXBWPl]+genPars[iSmp][iXBWMi]);
	      }
	    }
	      ...
	      for (uint iSmp=0; iSmp<genPars.size(); iSmp++) {
		genObs[iSmp][iT] = (genPars[iSmp][iXPl]-genPars[iSmp][iXMi])/(genPars[iSmp][iXPl]+genPars[iSmp][iXMi]);
	      }
	    }
	  }
	}
        ...
        for (uint iT=0; iT<nColT; iT++) {
          nomVecT[iT] = 0.0;
          for (uint iSmp=0; iSmp<genObs.size(); iSmp++) {
            nomVecT[iT] += genObs[iSmp][iT];
          }
          nomVecT[iT] *= (1./double(genObs.size()));
        }
	// Compute the covariance matrix
	for (uint i1=0; i1<nColT; i1++) {
	  for (uint i2=0; i2<nColT; i2++) {
	    covMatrixT[i1][i2] = 0.0;
	    for (uint iSmp=0; iSmp<genObs.size(); iSmp++) {
	      covMatrixT[i1][i2] += (genObs[iSmp][i1]-nomVecT[i1])*(genObs[iSmp][i2]-nomVecT[i2]);
	    }
	    covMatrixT[i1][i2] *= (1./(double(genObs.size())-1.));
	  }
	}
        //
        // Check the nominal value
        for (uint iT=0; iT<nColT; iT++) {
          if (genPass && std::abs((nomVecT[iT]-nomVec[iT])*100./nomVec[iT])>0.3) {
            std::cout << "[ERROR] Nominal value: " << nomVecT[iT] << " , " << nomVec[iT] << "  " << std::abs((nomVecT[iT]-nomVec[iT])*100./nomVec[iT]) << " are different for " << mod.first << "  " << var.first << "  " << chg.first << std::endl;
          }
        }
        // Check the covariance
        for (uint i1=0; i1<nColT; i1++) {
          for (uint i2=0; i2<nColT; i2++) {
            if (genPass && (std::abs(covMatrixT[i1][i2]-covMatrix[i1][i2])*100./std::sqrt(std::abs(covMatrix[i1][i1]*covMatrix[i2][i2])))>(i1==i2?15.0:30.0)) {
              std::cout << "[ERROR] Covariance value ("<<i1<<" , "<<i2<<") : " << covMatrixT[i1][i2] << " , " << covMatrix[i1][i2] << "  " << (std::abs(covMatrixT[i1][i2]-covMatrix[i1][i2])*100./std::sqrt(std::abs(covMatrix[i1][i1]*covMatrix[i2][i2]))) << " are different for " << mod.first << "  " << var.first << "  " << chg.first << std::endl;
            }
          }
        }
      }
  ...
  for (const auto& mod : CovMatrixObs) {
    ...
    for (const auto& var : mod.second) {
      for (const auto& chg : var.second) {
        ...
        if (covMatrix.Determinant()<=0.) { std::cout << "[ERROR] The determinat is: " << covMatrix.Determinant() << std::endl; return; } 
        // Compute the inverse
        TMatrixDSym invCovMatrix(covMatrix); invCovMatrix.Invert();
        // Compute the chi2
        double chi2 = 0.0;
        for (uint i1=0; i1 < nCol; i1++) {
          for (uint i2=0; i2 < nCol; i2++) {
